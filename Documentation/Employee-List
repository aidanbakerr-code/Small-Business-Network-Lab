# Employee List

## Company

**Baker Digital Solutions (BDS)**

## Employees

| Employee        | Job Title             | Department     | Primary Device   |
| --------------- | --------------------- | -------------- | ---------------- |
| Sarah Thompson  | Managing Director     | Management     | Laptop           |
| James Patel     | IT Administrator      | IT             | Desktop & Laptop |
| Emma Wilson     | Accounts Manager      | Finance        | Desktop          |
| Daniel Smith    | Sales Executive       | Sales          | Laptop           |
| Olivia Brown    | Marketing Coordinator | Marketing      | Desktop          |
| Michael Johnson | Customer Support      | Support        | Desktop          |
| Sophia Davis    | Receptionist          | Administration | Desktop          |

## Department Summary

| Department     | Number of Employees |
| -------------- | ------------------: |
| Management     |                   1 |
| IT             |                   1 |
| Finance        |                   1 |
| Sales          |                   1 |
| Marketing      |                   1 |
| Support        |                   1 |
| Administration |                   1 |

## Network Access Requirements

* All employees require internet access.
* All employees require access to the shared network printer.
* All employees require access to the shared file server.
* The IT Administrator requires administrative access to all network infrastructure.
* Guest devices must not have access to internal company resources.
